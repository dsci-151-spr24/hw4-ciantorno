...
gss16 <- gss16 %>%
  mutate(
    ...
    polviews = factor(polviews, levels = c("Conservative", "Moderate", "Liberal"))  
  )
...
```{r}
gss16_rec_1 <- recipe(advfront ~ polviews + wrkstat + educ, data = gss16) %>%
...
```{r}
set.seed(68)
gss16_folds <- vfold_cv(gss16, v = 5)
...
***The average area under the ROC curve is 0.62 and the accuracy for all cross validation folds is 0.84.***
...
***The average area under the ROC curve is 0.62 and the accuracy for all cross validation folds is 0.83.***
...
***Both models have about the same performance.  However, when looking at the full ROC AUC scores, model 1 performs slightly better.***
...
```{r}
gss16_preds_1 <- predict(gss16_fit_1, gss16_test, type = "prob") %>%
  bind_cols(gss16_test)

gss16_preds_2 <- predict(gss16_fit_2, gss16_test, type = "prob") %>%
  bind_cols(gss16_test)
```

```{r}
roc_1 <- roc_curve(gss16_preds_1, truth = advfront, .pred_Agree)
roc_2 <- roc_curve(gss16_preds_2, truth = advfront, .pred_Agree)
```

```{r}

roc_1 <- roc_1 %>% mutate(model = "Model 1")
roc_2 <- roc_2 %>% mutate(model = "Model 2")

roc_combined <- bind_rows(roc_1, roc_2)
...
```{r}
roc_auc(gss16_preds_1, truth = advfront, .pred_Agree)
roc_auc(gss16_preds_2, truth = advfront, .pred_Agree)
```

***The AUC's ended up showing that model two actually works slightly better, which is the opposite of what was displayed in the training data.***
...
***1136 chose "No" and 237 chose "Yes"***
...
***Bootstrapping can be used to estimate the proportion of harassed and non-harassed Americans by taking a lot of random samples and using them to calculate proportions, means, and other statistical data.  We can also make confidence intervals***
...
gss16_harass_filtered <- gss16 %>%
  filter(harass5 %in% c("Yes", "No"))


bootstrap_proportion <- function(data, n) {
...
bootstrap_results <- replicate(n_bootstrap, 
                               bootstrap_proportion(gss16_harass_filtered$harass5, 
                                                    length(gss16_harass_filtered$harass5)))


conf_int <- quantile(bootstrap_results, probs = c(0.025, 0.975))


conf_int
```

***This means that we are 95% sure that somewhere between 15% and 19% of Americans have experienced harassment from superiors at work.***
...
***I would expect that a 90% confidence interval would be wider than the interval calculated above, because the level of confidence is lower. Therefore, the answer less concise because there is a larger amount of possible percentages for the answer.***
